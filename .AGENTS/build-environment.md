...
- `FTK1011` with `The file exists` under `%LOCALAPPDATA%\tcs` can also be a junction-path FileTracker failure, not cache corruption. Confirmed locally: the same native target fails with tracking logs addressed through the junction and succeeds through the real path. For this signature, a process-local `$env:TrackFileAccess = 'false'` allows `test-local.ps1 -LibOnly` to compile and run tests without changing compiler settings. Restore the previous environment value afterward. This disables MSBuild file-access tracking for that invocation; do not persist it globally or use a shared `TLogLocation` across projects. Keep all process and running-app guards enabled.
