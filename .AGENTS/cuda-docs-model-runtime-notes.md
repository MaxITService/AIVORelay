...
- `cohere-fp16` uses the HF-style split-graph backend with a locally prepared mixed-precision package
...
- already-present local split-graph HF-style Cohere FP16 folders can also be auto-detected
...
- HF-style Cohere FP16 now reuses the same backend through a patched mixed-precision package boundary
- the branch still does not auto-download FP16; current FP16 support expects a prepared local model folder
