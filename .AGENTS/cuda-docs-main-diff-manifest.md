...
| `src-tauri/src/managers/model.rs` | CUDA branch carries the legacy Cohere Int8 catalog path and separate HF-style Cohere FP16/FP32 catalog and discovery behavior. |
| `src-tauri/src/managers/transcription.rs` | CUDA branch keeps the legacy Int8 Cohere load path separate from the HF-style Cohere backend used by FP16 and FP32 variants. |
| `src/i18n/locales/en/translation.json` | Adjusts CUDA-branch local-model strings, including Cohere Int8 and Cohere HF FP16/FP32 wording. |
| `src/i18n/locales/ru/translation.json` | Adjusts CUDA-branch local-model strings, including Cohere Int8 and Cohere HF FP16/FP32 wording. |
