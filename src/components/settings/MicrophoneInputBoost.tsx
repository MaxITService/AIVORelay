...
export const MicrophoneInputBoost: React.FC<MicrophoneInputBoostProps> =
  React.memo(
    ({
      ...
      const deviceKey =
        selectedMicrophone === "Default"
          ...
          : selectedMicrophone;
      const boostMap = ((settings as any)
        ?.microphone_input_boost_db_by_device ?? {}) as Record<string, number>;
      const microphoneInputBoostDb =
        boostMap[deviceKey] ??
        ((settings as any)?.microphone_input_boost_db as number | undefined) ??
        0;
        ...
        >
          <div className="w-full space-y-3">
            ...
            <input
              type="range"
              min="0"
              max={MAX_MICROPHONE_INPUT_BOOST_DB}
              step="1"
              value={displayValue}
              onChange={(e) => {
              ...
              style={{
                background: `linear-gradient(to right, #9b5de5 ${
                  (displayValue / MAX_MICROPHONE_INPUT_BOOST_DB) * 100
                }%, #252525 ${
                  (displayValue / MAX_MICROPHONE_INPUT_BOOST_DB) * 100
                }%)`,
              }}
            ...
            <div className="flex justify-between text-xs text-[#606060]">
              ...
              <span>{t("settings.sound.microphone.boost.max", "+17 dB")}</span>
            </div>
