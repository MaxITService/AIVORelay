...
| `src/soniox-live-preview/SonioxLivePreview.tsx`                        | **Soniox Live Preview Window**: Separate visual-only window that renders Soniox live final + interim text updates without affecting paste behavior, with live appearance updates (theme/opacity/colors). |
...
| `src/components/settings/user-interface/UserInterfaceSettings.tsx`       | Added Soniox live preview controls in User Interface settings (enable/disable, position, size, theme, transparency, font/accent colors, interim-text opacity). |
