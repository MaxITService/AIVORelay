...
struct PreviewDeleteLastWordShortcutAction;
...
pub(crate) fn force_post_process_for_next_transcription(binding_id: &str) {
    match FORCE_POST_PROCESS_BINDINGS.lock() {
        Ok(mut bindings) => {
            bindings.insert(binding_id.to_string());
        }
        Err(err) => {
            warn!(
                "Failed to mark forced post-processing for '{}': {}",
                binding_id, err
            );
        }
    }
...
fn take_force_post_process_for_binding(binding_id: &str) -> bool {
    FORCE_POST_PROCESS_BINDINGS
        .lock()
        .map(|mut bindings| bindings.remove(binding_id))
        .unwrap_or(false)
}
...
fn set_last_remote_recording_retry(request: RemoteRecordingRetryRequest) -> bool {
    match LAST_REMOTE_RECORDING_RETRY.lock() {
        Ok(mut slot) => {
            *slot = Some(RemoteRecordingRetryState::Ready(request));
            true
        }
        Err(err) => {
            warn!(
...
fn has_last_remote_recording_retry() -> bool {
    match LAST_REMOTE_RECORDING_RETRY.lock() {
        Ok(slot) => matches!(&*slot, Some(RemoteRecordingRetryState::Ready(_))),
        Err(err) => {
            warn!(
    ...
    match LAST_REMOTE_RECORDING_RETRY.lock() {
        Ok(mut slot) => match &*slot {
            Some(RemoteRecordingRetryState::Ready(req)) => {
                ...
                *slot = Some(RemoteRecordingRetryState::InFlight(req.retry_id));
                Ok(req_clone)
            }
            Some(RemoteRecordingRetryState::InFlight(_)) => {
                Err("A transcription retry is already in progress.".to_string())
            }
            None => Err("No failed Remote STT transcription is available to retry.".to_string()),
        },
        Err(err) => Err(format!("Failed to lock retry slot: {}", err)),
    }
}

fn restore_last_remote_recording_retry(
    retry_id: u64,
    request: RemoteRecordingRetryRequest,
) -> bool {
    if let Ok(mut slot) = LAST_REMOTE_RECORDING_RETRY.lock() {
        if let Some(RemoteRecordingRetryState::InFlight(id)) = &*slot {
            if *id == retry_id {
                *slot = Some(RemoteRecordingRetryState::Ready(request));
                return true;
            }
...
fn clear_last_remote_recording_retry_by_id(retry_id: u64) {
    if let Ok(mut slot) = LAST_REMOTE_RECORDING_RETRY.lock() {
        if let Some(state) = &*slot {
            let matches = match state {
                RemoteRecordingRetryState::Ready(req) => req.retry_id == retry_id,
                RemoteRecordingRetryState::InFlight(id) => *id == retry_id,
            };
            if matches {
                *slot = None;
            }
...
pub fn clear_ready_remote_recording_retry_by_id(retry_id: u64) {
    if let Ok(mut slot) = LAST_REMOTE_RECORDING_RETRY.lock() {
        if let Some(RemoteRecordingRetryState::Ready(req)) = &*slot {
            if req.retry_id != retry_id {
                return;
            }
            *slot = None;
        }
...
pub fn clear_any_ready_remote_recording_retry() {
    if let Ok(mut slot) = LAST_REMOTE_RECORDING_RETRY.lock() {
        if matches!(&*slot, Some(RemoteRecordingRetryState::Ready(_))) {
            *slot = None;
        }
    }
}
...
fn handle_remote_transcription_error(app: &AppHandle, err_str: &str, include_retry_action: bool) {
    ...
    if include_retry_action {
        crate::plus_overlay_state::handle_transcription_error_with_retry(app, err_str, None);
    } else {
        crate::plus_overlay_state::handle_transcription_error(app, err_str);
    }
}

fn handle_remote_transcription_error_with_retry_session(
    app: &AppHandle,
    err_str: &str,
    retry_id: u64,
) -> bool {
    let retry_visible = crate::plus_overlay_state::handle_transcription_error_with_retry(
        app,
        err_str,
        Some(retry_id),
    );
...
fn set_deepgram_stream_emitted(binding_id: &str, emitted: bool) {
    if let Ok(mut map) = DEEPGRAM_STREAM_EMITTED.lock() {
        map.insert(binding_id.to_string(), emitted);
    }
}

fn mark_deepgram_stream_emitted(binding_id: &str) {
    set_deepgram_stream_emitted(binding_id, true);
}
...
fn resolve_effective_language(
    ...
    let Some(model_info) = mm.get_model_info(&settings.selected_model) else {
        return requested;
    };

    model::effective_language(
        &requested,
...
async fn maybe_post_process_transcription(
    ...
    if !force_manual {
        ...
        if settings.transcription_provider == TranscriptionProvider::RemoteDeepgram
            && settings.deepgram_live_enabled
        {
            debug!("Skipping post-processing for Deepgram live transcription");
            return PostProcessTranscriptionOutcome::Skipped;
        }
    }
...
fn show_ai_replace_error_overlay(app: &AppHandle, message: impl Into<String>) {
    ...
    let settings = get_settings(app);
    crate::plus_overlay_state::show_categorized_ai_replace_error_overlay(
        app,
        &message,
        settings.ai_replace_provider_id.is_none(),
    );
}
...
fn start_recording_with_feedback(app: &AppHandle, binding_id: &str) -> bool {
    ...
    let native_stream_language = captured_profile
        ...
        .unwrap_or_else(|| settings.selected_language.clone());
    let native_stream_translate = captured_profile
    ...
    *state_guard = session_manager::SessionState::Recording {
    ...
    if recording_started {
        ...
        if !session_still_recording {
            ...
            rm.cancel_recording();
            if use_live_streaming || use_native_local_streaming {
...
struct StoppedTranscribeRecording {
    samples: Vec<f32>,
    pre_saved_file_name: Option<String>,
    post_process_requested: bool,
    quick_tap_skipped: bool,
}
...
async fn perform_transcription_for_profile_with_retry_action(
    ...
    if settings.transcription_provider == TranscriptionProvider::RemoteOpenAiCompatible {
        ...
        let result = if should_stream_insert {
                ...
                .transcribe_with_streaming_callback(
                    ...
                    move |chunk| {
                        ...
                        let delta = match stream_processor_for_callback.lock() {
                            ...
                            Err(_) => {
                                return Err(anyhow::anyhow!(
                                    "Failed to lock Soniox stream processor"
                                ));
                            }
                        };
                        if delta.is_empty() {
                        ...
                        ah_for_call
                            ...
                            .map_err(|e| {
                                anyhow::anyhow!("Failed to queue stream chunk paste: {}", e)
                            })
                    },
                )
                .await;
            ...
            match streamed_result {
                Ok(text) => match stream_processor.lock() {
                    Ok(mut processor) => {
    ...
    } else if settings.transcription_provider == TranscriptionProvider::RemoteDeepgram {
        let language = profile
            .as_ref()
            .map(|p| p.language.clone())
            .unwrap_or_else(|| settings.selected_language.clone());
        ...
        if let Some(p) = &profile {
            log::info!(
                "Transcription using Deepgram with profile '{}' (lang={}): model={}",
                p.name,
                language,
            ...
            log::info!(
                "Transcription using Deepgram: model={}, lang={}",
                settings.deepgram_model,
                language
            );
        }
        ...
        if deepgram_manager.is_cancelled(operation_id) {
            debug!(
                "Deepgram transcription operation {} was cancelled, discarding result",
                operation_id
            );
            return TranscriptionOutcome::Cancelled;
        }
    ...
    if let Some(samples) = rm.stop_recording(binding_id) {
        ...
        .await
        {
            TranscriptionOutcome::Success(text) => {
                TranscriptionFetchOutcome::Success((text, samples))
            }
...
fn save_failed_transcription_entry(
    ...
    let hm = Arc::clone(&app.state::<Arc<HistoryManager>>());
    match hm.save_entry(file_name, String::new(), post_process_requested, None, None) {
...
async fn stop_recording_for_transcribe_action(
    app: &AppHandle,
    binding_id: &str,
    captured_profile_id: Option<&String>,
    recording_settings: &AppSettings,
) -> Option<StoppedTranscribeRecording> {
    let rm = Arc::clone(&app.state::<Arc<AudioRecordingManager>>());
    let is_soniox_optimized_delivery =
        should_use_soniox_optimized_delivery(recording_settings, binding_id, captured_profile_id);
    let should_clear_local_preview_callback = recording_settings.transcription_provider
        == TranscriptionProvider::Local
        && should_route_output_to_preview_for_captured_profile(
            recording_settings,
            captured_profile_id,
        );
    ...
    let samples = match rm.stop_recording(binding_id) {
        Some(samples) => {
            if is_soniox_optimized_delivery
                || should_clear_local_preview_callback
                || app
    ...
    if should_skip_transcription_for_quick_tap(binding_id, recording_settings, samples.len()) {
        ...
        app.state::<Arc<TranscriptionManager>>().cancel_stream();
        return Some(StoppedTranscribeRecording {
            samples,
            pre_saved_file_name: None,
            post_process_requested: false,
            quick_tap_skipped: true,
        });
    }
    ...
    Some(StoppedTranscribeRecording {
        samples,
        pre_saved_file_name,
        post_process_requested,
        quick_tap_skipped: false,
    })
}

async fn transcribe_stopped_recording_for_transcribe_action(
    app: &AppHandle,
    binding_id: &str,
    captured_profile_id: Option<String>,
    recording_settings: AppSettings,
    current_app: String,
    stopped: StoppedTranscribeRecording,
) -> Option<(String, Vec<f32>, Option<String>, bool)> {
    let samples = stopped.samples;
    let pre_saved_file_name = stopped.pre_saved_file_name;
    let post_process_requested = stopped.post_process_requested;
    ...
    .await
    {
        ...
        TranscriptionOutcome::Error {
            ...
            let mut retry_error_shown = false;
            if remote_retry_enabled {
                if let Some(file_name) = pre_saved_file_name.clone() {
                    let failed_id =
                        save_failed_transcription_entry(app, file_name, post_process_requested);
                    if let Some(failed_history_entry_id) = failed_id {
                        let retry_id = RETRY_ID_COUNTER.fetch_add(1, AtomicOrdering::SeqCst);
                        let request = RemoteRecordingRetryRequest {
                            retry_id,
                            ...
                            failed_history_entry_id,
                        };

                        if set_last_remote_recording_retry(request) {
                            retry_error_shown =
                                handle_remote_transcription_error_with_retry_session(
                                    app, &message, retry_id,
                                );
                        } else {
                            handle_remote_transcription_error(app, &message, false);
    ...
    tauri::async_runtime::spawn(async move {
        ...
        let transcription = match outcome {
            ...
            TranscriptionOutcome::Error { message, .. } => {
                ...
                if restore_last_remote_recording_retry(retry_id, request) {
                    handle_remote_transcription_error_with_retry_session(&app, &message, retry_id);
                }
                return;
            }
        };
...
fn should_route_output_to_preview(
    settings: &AppSettings,
    profile: Option<&TranscriptionProfile>,
) -> bool {
    ...
    if explicit_preview_workflow {
        return true;
    }

    // The global live preview window is not just visual: for live providers it
    // routes output through the preview workflow until stop/finalize.
    should_route_live_output_to_preview(settings)
}

fn should_route_output_to_preview_for_captured_profile(
    settings: &AppSettings,
    captured_profile_id: Option<&String>,
) -> bool {
...
fn local_model_supports_native_streaming(app: &AppHandle, settings: &AppSettings) -> bool {
    if settings.transcription_provider != TranscriptionProvider::Local {
        return false;
    }
    ...
    app.state::<Arc<ModelManager>>()
        ...
        .map(|model| {
            matches!(model.engine_type, EngineType::TranscribeCpp) && model.supports_streaming
        })
        .unwrap_or(false)
}
...
fn is_preview_sentence_boundary_char(ch: char) -> bool {
    matches!(ch, '.' | '!' | '?' | '…')
}

fn is_preview_delete_boundary_char(ch: char, boundary: PreviewDeleteMode) -> bool {
    is_preview_sentence_boundary_char(ch)
        || matches!(boundary, PreviewDeleteMode::DotOrComma) && ch == ','
}

fn preview_boundary_end_indices(text: &str, boundary: PreviewDeleteMode) -> Vec<usize> {
    let mut ends = Vec::new();
    let mut current_cluster_end: Option<usize> = None;

    for (idx, ch) in text.char_indices() {
        if is_preview_delete_boundary_char(ch, boundary) {
            current_cluster_end = Some(idx);
        } else if let Some(cluster_end) = current_cluster_end.take() {
            ends.push(cluster_end);
        }
    }

    if let Some(cluster_end) = current_cluster_end {
        ends.push(cluster_end);
    }
...
fn trim_preview_text_to_boundary(text: &str, boundary: PreviewDeleteMode) -> String {
    let trimmed = text.trim_end_matches(char::is_whitespace);
    if trimmed.is_empty() {
        return String::new();
    }

    let boundary_ends = preview_boundary_end_indices(trimmed, boundary);
    let ends_with_boundary = trimmed
        .chars()
        .last()
        .map(|ch| is_preview_delete_boundary_char(ch, boundary))
        .unwrap_or(false);

    let keep_boundary_idx = if ends_with_boundary {
        boundary_ends
            .len()
            .checked_sub(2)
            .and_then(|idx| boundary_ends.get(idx).copied())
    } else {
        boundary_ends.last().copied()
    };

    let Some(keep_boundary_idx) = keep_boundary_idx else {
        return String::new();
    };

    let boundary_char_len = trimmed[keep_boundary_idx..]
        .chars()
        .next()
        .map(|ch| ch.len_utf8())
        .unwrap_or(0);
    if boundary_char_len == 0 {
        return String::new();
    }

    let kept = &trimmed[..keep_boundary_idx + boundary_char_len];
    if kept.is_empty() {
        String::new()
    } else {
        format!("{} ", kept)
...
fn trim_preview_text_last_word(text: &str) -> String {
    let trimmed = text.trim_end_matches(char::is_whitespace);
    if trimmed.is_empty() {
        return String::new();
    }

    let mut last_non_whitespace_start: Option<usize> = None;
    for (idx, ch) in trimmed.char_indices().rev() {
        if !ch.is_whitespace() {
            last_non_whitespace_start = Some(idx);
            continue;
        }
        if last_non_whitespace_start.is_some() {
            let kept = trimmed[..idx].trim_end_matches(char::is_whitespace);
...
fn trim_preview_text_last_char(text: &str) -> String {
    // Trim trailing whitespace first, then drop exactly one Unicode scalar.
    let trimmed = text.trim_end_matches(char::is_whitespace);
    if trimmed.is_empty() {
        return String::new();
    }
    let mut chars = trimmed.chars();
    chars.next_back(); // drop the last char
    let kept = chars.as_str().trim_end_matches(char::is_whitespace);
    if kept.is_empty() {
        String::new()
    } else {
        format!("{} ", kept)
...
fn trim_preview_text(text: &str, mode: PreviewDeleteMode) -> String {
    match mode {
        PreviewDeleteMode::DotOrComma | PreviewDeleteMode::Dot => {
            trim_preview_text_to_boundary(text, mode)
        }
        PreviewDeleteMode::LastWord => trim_preview_text_last_word(text),
        PreviewDeleteMode::LastChar => trim_preview_text_last_char(text),
    }
}

fn maybe_arm_decapitalize_after_preview_delete(
    app: &AppHandle,
    mode: PreviewDeleteMode,
    preview_is_realtime: bool,
    did_delete: bool,
) {
    if !matches!(
        mode,
        PreviewDeleteMode::LastWord | PreviewDeleteMode::LastChar
    ) || !did_delete
    {
        return;
    }
...
fn drop_preview_recording_without_finalize(
    ...
    let Some(session) = crate::session_manager::take_session_if_matches(app, binding_id) else {
        return Err("No active recording session to drop.".to_string());
    };
...
async fn preview_delete_action(app: AppHandle, mode: PreviewDeleteMode) -> Result<(), String> {
    ...
    let binding_id = state.binding_id.ok_or_else(|| {
        "No active transcription binding for Output to Preview workflow.".to_string()
    })?;

    let was_recording = is_recording_for_binding(&app, &binding_id);
    // LastChar only trims finalized text — never interrupt an active recording session.
    if was_recording && mode != PreviewDeleteMode::LastChar {
        let interim_visible = !current_preview_interim_text().trim().is_empty();
        ...
        if interim_visible {
            ...
            if !wait_for_session_idle(&app, Duration::from_secs(20)).await {
                let err = "Timed out while finalizing active recording.".to_string();
                crate::managers::preview_output_mode::set_error(&app, Some(err.clone()));
                return Err(err);
            }
        } else if let Err(err) = drop_preview_recording_without_finalize(&app, &binding_id) {
            crate::managers::preview_output_mode::set_error(&app, Some(err.clone()));
            return Err(err);
        }
    }

    let current_text = current_preview_buffer_text();
    let next_text = trim_preview_text(&current_text, mode);
    replace_preview_text_for_output_mode(&app, &next_text);
    maybe_arm_decapitalize_after_preview_delete(
        &app,
        mode,
        state.is_realtime,
...
pub(crate) async fn process_live_sound_transcription_text(app: AppHandle) -> Result<(), String> {
    ...
    let result =
        match maybe_post_process_transcription(&app, &settings, profile, &template_context, true)
            .await
        {
...
fn should_use_live_for_recording(app: &AppHandle, binding_id: &str) -> bool {
    ...
    match &*state_guard {
        session_manager::SessionState::Recording {
            ...
            captured_settings,
            ..
        } if current_binding_id == binding_id => match captured_settings.transcription_provider {
            TranscriptionProvider::RemoteSoniox => {
                let soniox_live_manager = Arc::clone(&app.state::<Arc<SonioxRealtimeManager>>());
                soniox_live_manager.has_active_session()
...
/// Starts provider-specific live session for an action.
...
fn setup_and_start_live(
    ...
    match settings.transcription_provider {
        ...
        _ => Err("Live streaming is not enabled for this provider".to_string()),
    }
}

fn resolve_soniox_hint_from_language(language: &str) -> Option<String> {
    let resolution =
        crate::language_resolver::resolve_requested_language_for_soniox(Some(language));
...
fn resolve_stream_trailing_adjustment(
    ...
    if adjusted_trailing_count > original_trailing_count {
        ...
        StreamTrailingAdjustment::RemoveCharacters(
            original_trailing_count - adjusted_trailing_count,
        )
    } else {
...
impl ShortcutAction for TranscribeAction {
    fn start(&self, app: &AppHandle, binding_id: &str, _shortcut_str: &str) {
        ...
        if use_live_streaming {
            ...
            match settings.transcription_provider {
                TranscriptionProvider::RemoteSoniox => {
                    let soniox_live_manager =
                        Arc::clone(&app.state::<Arc<SonioxRealtimeManager>>());
                    #[cfg(target_os = "windows")]
                    let api_key = crate::secure_keys::get_soniox_api_key();
                    #[cfg(not(target_os = "windows"))]
                    let api_key = String::new();
                    ...
                    if let Err(err) = start_result {
                        ...
                        debug!("Soniox live session started for binding '{}'", binding_id);
                    }
                }
                TranscriptionProvider::RemoteDeepgram => {
                    let deepgram_live_manager =
                    ...
                    set_deepgram_stream_emitted(&binding_id, false);
                    let chunk_callback = stream_processor.map(|stream_processor| {
                        Arc::new({
                            ...
                            move |chunk: String| {
                                ...
                                let delta = match stream_processor.lock() {
                                    Ok(mut processor) => processor.push_chunk(&chunk),
                                    Err(_) => {
                                        warn!("Failed to lock Deepgram stream processor");
                                        String::new()
                                    }
                                };
                                if delta.is_empty() {
                                ...
                                mark_deepgram_stream_emitted(&binding_id_for_cb);
                                let ah_for_call = ah_for_cb.clone();
                                let ah_for_clip = ah_for_call.clone();
                                let _ = ah_for_call.run_on_main_thread(move || {
                                    let _ = crate::clipboard::paste_stream_chunk(
                        ...
                        }) as DeepgramFinalChunkCallback
                    });

                    let start_result = deepgram_live_manager.start_session(
                        &binding_id,
                        &api_key,
                        &settings.deepgram_model,
    ...
    fn stop(&self, app: &AppHandle, binding_id: &str, shortcut_str: &str) {
        ...
        tauri::async_runtime::spawn(async move {
            ...
            clear_last_remote_recording_retry();
            let stopped = match stop_recording_for_transcribe_action(
                &ah,
                &binding_id,
                captured_profile_id.as_ref(),
                &recording_settings,
                ...
                && !stopped.quick_tap_skipped
            {
                ...
                if tm.is_streaming() || tm.stream_router().is_open() {
            ...
            let (transcription, samples, pre_saved_file_name, post_process_requested) =
                if stopped.quick_tap_skipped {
                    (
                        String::new(),
                        stopped.samples,
                        stopped.pre_saved_file_name,
                        stopped.post_process_requested,
                    )
                } else if let Some(text) = native_stream_text {
                    (
                        text,
                        stopped.samples,
                        stopped.pre_saved_file_name,
                    ...
                    match transcribe_stopped_recording_for_transcribe_action(
                        &ah,
                        &binding_id,
                        captured_profile_id,
                        recording_settings.clone(),
...
impl ShortcutAction for SendToExtensionAction {
    fn start(&self, app: &AppHandle, binding_id: &str, _shortcut_str: &str) {
        ...
        if use_soniox_live {
            if let Err(err) = setup_and_start_live(app, &settings, binding_id) {
    ...
    fn stop(&self, app: &AppHandle, binding_id: &str, _shortcut_str: &str) {
        ...
        tauri::async_runtime::spawn(async move {
            ...
            let final_voice_text = if voice_text.trim().is_empty() {
                ...
                recording_settings
                    .screenshot_no_voice_default_prompt
                    .clone()
            } else {
                voice_text
            };
            ...
            let existing_files = collect_existing_images(
                &screenshot_folder,
        ...
        tauri::async_runtime::spawn(async move {
            ...
            match hm.get_latest_entry() {
                Ok(Some(entry)) => {
                    ...
                    tokio::time::sleep(Duration::from_millis(REPASTE_LAST_PRE_PASTE_DELAY_MS))
                        .await;
    ...
    fn start(&self, app: &AppHandle, _binding_id: &str, _shortcut_str: &str) {
        ...
        tauri::async_runtime::spawn(async move {
            if let Err(err) = preview_delete_last_word_action(ah).await {
                warn!("Preview delete-last-word shortcut failed: {}", err);
            }
        });
    }
...
struct RemoteRecordingRetryRequest {
    ...
    current_app: String,
    pre_saved_file_name: Option<String>,
    failed_history_entry_id: i64,
}

#[derive(Clone)]
enum RemoteRecordingRetryState {
    Ready(RemoteRecordingRetryRequest),
    InFlight(u64),
}

static LAST_REMOTE_RECORDING_RETRY: Lazy<Mutex<Option<RemoteRecordingRetryState>>> =
    Lazy::new(|| Mutex::new(None));

use std::sync::atomic::{AtomicU64, Ordering as AtomicOrdering};
static RETRY_ID_COUNTER: AtomicU64 = AtomicU64::new(1);
...
pub async fn preview_llm_process_action(app: AppHandle) -> Result<(), String> {
    ...
    let should_resume_recording =
        was_recording && crate::managers::preview_output_mode::is_active_for_binding(&binding_id);
...
#[tauri::command]
#[specta::specta]
pub async fn preview_delete_until_dot_or_comma_action(app: AppHandle) -> Result<(), String> {
    preview_delete_action(app, PreviewDeleteMode::DotOrComma).await
}

#[tauri::command]
#[specta::specta]
pub async fn preview_delete_until_dot_action(app: AppHandle) -> Result<(), String> {
    preview_delete_action(app, PreviewDeleteMode::Dot).await
}

#[tauri::command]
#[specta::specta]
pub async fn preview_delete_last_word_action(app: AppHandle) -> Result<(), String> {
    preview_delete_action(app, PreviewDeleteMode::LastWord).await
}

#[tauri::command]
#[specta::specta]
pub async fn preview_delete_last_char_action(app: AppHandle) -> Result<(), String> {
    // Single-character deletion: only operates on finalized text,
    // does not drop the live tail when recording is active.
    preview_delete_action(app, PreviewDeleteMode::LastChar).await
}
...
pub static ACTION_MAP: Lazy<HashMap<String, Arc<dyn ShortcutAction>>> = Lazy::new(|| {
    ...
    map.insert(
        crate::settings::PREVIEW_DELETE_LAST_WORD_BINDING_ID.to_string(),
        Arc::new(PreviewDeleteLastWordShortcutAction) as Arc<dyn ShortcutAction>,
    );
    #[cfg(target_os = "windows")]
    map.insert(
