...
/// Intentionally bounded and lossy: this connector is not a guaranteed-delivery
/// store. If the extension stays offline long enough, older undelivered entries
/// may be dropped on purpose so memory stays bounded and fresh messages win.
/// Keepalives share the same queue intentionally, so prolonged offline periods
/// naturally age out stale backlog instead of preserving it forever.
...
const LEGACY_SHARED_CONNECTOR_PASSWORD: &str =
    "befc3aa14cc05e56011865df1c49d16ef9100a53d9bfa02be8d4ffd386324f65";
...
const CONNECTOR_PASSWORD_AUTH_CONTEXT: &[u8] = b"AivoRelay Connector Protocol v3 password auth key";
...
impl ConnectorManager {
    pub fn new(app_handle: &AppHandle) -> Result<Self, String> {
        ...
        replace_missing_or_legacy_connector_password(app_handle, &settings)?;
        ...
        std::thread::spawn(move || {
            ...
            while manager.server_running.load(Ordering::SeqCst) {
                if !delay_warning_emitted
                    && started_waiting_at.elapsed()
                        >= Duration::from_millis(RESTART_DELAY_WARNING_MS)
                {
                    let _ = manager.app_handle.emit("connector-restart-delayed", true);
...
async fn handle_get_messages(
    ...
    let password_update = pending_password_update(&app_state.app_handle);
    let auto_open_url =
...
async fn handle_create_session(
    ...
    let cors_policy =
        match parse_cors_policy(&settings.connector_cors, settings.connector_allow_any_cors) {
            Ok(policy) => policy,
    ...
    let client_public_key = match PublicKey::from_sec1_bytes(&client_public_key_bytes) {
        Ok(key) => key,
        Err(_) => return error_response(StatusCode::BAD_REQUEST, "Malformed client public key"),
    };
    ...
    let server_public_key_bytes = server_public_key
    ...
    let auth_key = derive_password_auth_key(match auth_match {
        ...
        AuthMatch::PendingPassword => settings
            .connector_pending_password
            .as_deref()
            .unwrap_or(&settings.connector_password),
    });
...
fn register_auth_failure(app_state: &AppState) -> i64 {
    ...
    let delay_ms = (INITIAL_AUTH_BACKOFF_MS * (1_i64 << exponent)).min(MAX_AUTH_BACKOFF_MS);
...
fn validate_origin_header(
    ...
    let origin = match headers.get(header::ORIGIN) {
        Some(origin_value) => origin_value
            .to_str()
            .map_err(|_| error_response(StatusCode::BAD_REQUEST, "Malformed Origin header"))?,
        None => "",
    };
    ...
    match cors_policy {
        ...
        CorsPolicy::Exact(expected_origin) => {
            if origin.is_empty() && expected_origin.starts_with("chrome-extension://") {
                let extension_id =
                    required_string_header(headers, HEADER_EXTENSION_ID, "extension id")?;
                let expected_extension_id =
                    expected_origin.trim_start_matches("chrome-extension://");
...
fn generate_random_bytes(byte_len: usize) -> Option<Vec<u8>> {
    ...
    if let Err(err) = getrandom::getrandom(&mut bytes) {
        error!(
            "Failed to generate secure random bytes from OS CSPRNG: {}",
            err
        );
        return None;
    }
...
fn pending_password_update(app: &AppHandle) -> Option<String> {
...
fn replace_missing_or_legacy_connector_password(
    ...
    if !settings.connector_password.is_empty()
        && settings.connector_password.as_str() != LEGACY_SHARED_CONNECTOR_PASSWORD
    {
        return Ok(());
    }

    let new_password = generate_secure_password().ok_or_else(|| {
        "Failed to generate a unique connector password; connector startup aborted".to_string()
    })?;
    info!("Replacing missing or legacy shared connector password with a unique secret");

    let mut new_settings = settings.clone();
    new_settings.connector_password = new_password.into();
...
fn clear_pending_password(app: &AppHandle, settings: &crate::settings::AppSettings) {
    ...
    if let Some(connector_manager) = app.try_state::<Arc<ConnectorManager>>() {
        connector_manager.refresh_crypto_state(&new_settings.connector_password, None);
    }
}

pub(crate) fn active_pending_password(
    app: &AppHandle,
    settings: &crate::settings::AppSettings,
) -> Option<String> {
    let pending = settings.connector_pending_password.as_ref()?;
    let issued_at = settings.connector_pending_password_issued_at_ms;
    if issued_at > 0 && now_ms() - issued_at > PENDING_PASSWORD_TTL_MS {
        info!("Pending connector password expired after TTL; clearing it");
...
fn json_session_response<T: Serialize>(
    ...
    match serde_json::to_vec(&payload) {
        ...
        Err(err) => {
            error!(
                "Failed to serialize connector session JSON response: {}",
                err
            );
            error_response(
                StatusCode::INTERNAL_SERVER_ERROR,
